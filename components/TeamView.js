...
export default function TeamView({ user }) {
  ...
  const getRoleBadge = (role) => {
    if (role === 'super_admin') return <Badge className="bg-amber-100 text-amber-800 text-xs"><Crown size={10} className="mr-1" /> Super Admin</Badge>;
    if (role === 'admin') return <Badge className="bg-blue-100 text-blue-800 text-xs"><Shield size={10} className="mr-1" /> Admin</Badge>;
    return <Badge className="bg-gray-100 text-gray-700 text-xs"><User size={10} className="mr-1" /> Custom Access</Badge>;
  };
  ...
  const canManageMember = (member) => {
    ...
    if (isAdmin && member.role !== 'super_admin') return true;
    return false;
  };
        ...
        {canManageTeam && (
          <Dialog open={showDialog} onOpenChange={(open) => { setShowDialog(open); if (!open) { setEditingMember(null); resetForm(); } }}>
            <DialogTrigger asChild>
              <Button size="sm" className="w-full sm:w-auto"><UserPlus size={16} className="mr-2" /> Add Member</Button>
            </DialogTrigger>
            <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
              ...
              <div className="space-y-4 mt-4">
                ...
                {isSuperAdmin && !editingMember && organizations.length > 0 && (
                  <div className="space-y-2">
                    <Label className="flex items-center gap-2">
                      <Building2 size={14} /> Assign to Org(s) *
                    </Label>
                    <div className="border rounded-lg p-2 sm:p-3 space-y-2 max-h-32 sm:max-h-40 overflow-y-auto bg-muted/30">
                      {organizations.map(org => (
                        <div 
                          key={org.id} 
                          className={`flex items-center gap-2 p-2 rounded-md cursor-pointer transition-colors ${form.organizationIds?.includes(org.id) ? 'bg-blue-50 border border-blue-200' : 'hover:bg-muted'}`}
                          onClick={() => toggleOrg(org.id)}
                        >
                          <Checkbox checked={form.organizationIds?.includes(org.id)} onCheckedChange={() => toggleOrg(org.id)} />
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium truncate">{org.name}</p>
                            {org.industry && <p className="text-xs text-muted-foreground">{org.industry}</p>}
                          </div>
                          {form.organizationIds?.includes(org.id) && <Check size={14} className="text-blue-600 flex-shrink-0" />}
                        </div>
                      ))}
                    </div>
                    {form.organizationIds?.length > 0 && <p className="text-xs text-muted-foreground">{form.organizationIds.length} org(s) selected</p>}
                  </div>
                )}
                ...
                <div>
                  ...
                  <Select value={form.role} onValueChange={v => setForm({ ...form, role: v })}>
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="admin"><div className="flex items-center gap-2"><Shield size={14} className="text-blue-600" /> Admin</div></SelectItem>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div><Label className="text-xs">Designation</Label><Input value={form.designation} onChange={e => setForm({ ...form, designation: e.target.value })} placeholder="Designer" /></div>
                  <div><Label className="text-xs">Department</Label><Input value={form.department} onChange={e => setForm({ ...form, department: e.target.value })} placeholder="Creative" /></div>
                </div>
                <Button onClick={handleSave} className="w-full">{editingMember ? 'Update' : 'Add'} Member</Button>
              </div>
            </DialogContent>
          </Dialog>
        )}
            ...
            <Card key={m.id} className="border shadow-sm hover:shadow-md transition-shadow">
              <CardContent className="p-4 sm:pt-6">
                <div className="flex items-start gap-3">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${m.role === 'super_admin' ? 'bg-gradient-to-br from-amber-500 to-orange-500' : 'bg-gradient-to-br from-blue-500 to-indigo-500'}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between">
                      ...
                      {canManageMember(m) && (
                        <div className="flex gap-0.5 flex-shrink-0 ml-2">
                          <button onClick={() => handleEdit(m)} className="p-1 sm:p-1.5 rounded hover:bg-muted" title="Edit"><Pencil size={12} /></button>
                          <button onClick={() => { setShowPasswordDialog(m); setNewPassword(''); }} className="p-1 sm:p-1.5 rounded hover:bg-muted" title="Reset Password"><KeyRound size={12} /></button>
                          <button onClick={() => handleDelete(m)} className="p-1 sm:p-1.5 rounded hover:bg-red-50 text-red-500" title="Delete"><Trash2 size={12} /></button>
                        </div>
                      )}
                    </div>
                    ...
                    {(m.designation || m.department) && (
                      <div className="flex flex-wrap items-center gap-2 mt-2 text-xs text-muted-foreground">
                        {m.designation && <span className="flex items-center gap-1"><Briefcase size={10} /> {m.designation}</span>}
                        {m.department && <span className="flex items-center gap-1"><Building2 size={10} /> {m.department}</span>}
                      </div>
                    )}
                    {isSuperAdmin && m.organizationIds && m.organizationIds.length > 0 && (
                      <div className="flex flex-wrap gap-1 mt-2">
                        {m.organizationIds.slice(0, 2).map(oid => (
                          <span key={oid} className="text-xs bg-muted px-1.5 py-0.5 rounded-full flex items-center gap-1">
                            <Building2 size={9} /> {getOrgName(oid) || 'Org'}
                          </span>
                        ))}
                        {m.organizationIds.length > 2 && <span className="text-xs text-muted-foreground">+{m.organizationIds.length - 2}</span>}
                      </div>
                    )}
                  </div>
                </div>
