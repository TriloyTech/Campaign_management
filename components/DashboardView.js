    ...
    <div className="space-y-6">
      ...
      {isAdmin && financials && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Card className="border-0 shadow-sm"><CardContent className="pt-6"><div className="flex items-center justify-between"><div><p className="text-sm text-muted-foreground">Earned Revenue</p><p className="text-2xl font-bold text-emerald-600">{formatBDT(financials.totalEarned)}</p></div><div className="w-10 h-10 bg-emerald-100 rounded-lg flex items-center justify-center"><DollarSign className="text-emerald-600" size={20} /></div></div><p className="text-xs text-emerald-600 mt-2">{financials.totalProjected > 0 ? Math.round((financials.totalEarned / financials.totalProjected) * 100) : 0}% of projected</p></CardContent></Card>
