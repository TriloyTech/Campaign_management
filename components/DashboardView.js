...
import { TrendingUp, DollarSign, Clock, CheckCircle, BarChart3, Activity, AlertCircle } from 'lucide-react';
...
export default function DashboardView({ user, navigate }) {
  ...
  const loadDashboard = async () => {
    try {
      const [dashRes, chartRes] = await Promise.all([
        ...
        user.role === 'admin' ? apiFetch('GET', 'dashboard/revenue-chart') : Promise.resolve({ chartData: [] })
      ]);
      setData(dashRes);
  ...
  const progressPercent = deliverableStats?.total > 0
    ? Math.round((deliverableStats.delivered / deliverableStats.total) * 100) : 0;
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">{isAdmin ? 'Agency revenue overview' : 'Your campaign overview'}</p>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-slate-900">{formatBDT(financials.totalProjected)}</p>
                </div>
                <div className="w-10 h-10 bg-blue-100 rounded-lg flex items-center justify-center">
                  <TrendingUp className="text-blue-600" size={20} />
                </div>
              </div>
              <p className="text-xs text-muted-foreground mt-2">{financials.activeCampaigns} active campaigns</p>
            </CardContent>
          </Card>
            ...
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-emerald-600">{formatBDT(financials.totalEarned)}</p>
                </div>
                <div className="w-10 h-10 bg-emerald-100 rounded-lg flex items-center justify-center">
                  <DollarSign className="text-emerald-600" size={20} />
                </div>
              </div>
              <p className="text-xs text-emerald-600 mt-2">{financials.totalProjected > 0 ? Math.round((financials.totalEarned / financials.totalProjected) * 100) : 0}% of projected</p>
            </CardContent>
          </Card>
            ...
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-amber-600">{formatBDT(financials.totalPending)}</p>
                </div>
          ...
          <Card className="border-0 shadow-sm">
            <CardContent className="pt-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm text-muted-foreground">Deliverables Done</p>
                  <p className="text-2xl font-bold">{deliverableStats?.delivered || 0}/{deliverableStats?.total || 0}</p>
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="border-0 shadow-sm">
          <CardHeader>
            <CardTitle className="text-base flex items-center gap-2">
              <Activity size={18} /> Deliverable Status
            </CardTitle>
          </CardHeader>
          <CardContent>
            {pieData.length > 0 ? (
      ...
      {/* Client Breakdown & Recent Activity */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {isAdmin && clientBreakdown?.length > 0 && (
          <Card className="border-0 shadow-sm">
            <CardHeader>
              <CardTitle className="text-base">Client Revenue Breakdown</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {clientBreakdown.map((c, i) => (
                  <div key={i} className="space-y-2">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="text-muted-foreground">{formatBDT(c.earned)} / {formatBDT(c.projected)}</span>
                    </div>
                    <div className="w-full bg-gray-100 rounded-full h-2">
                      <div className="bg-blue-500 h-2 rounded-full transition-all" style={{ width: `${c.projected > 0 ? (c.earned / c.projected * 100) : 0}%` }} />
                    </div>
                  </div>
                ))}
              </div>
      ...
      {campaigns?.length > 0 && (
        <Card className="border-0 shadow-sm">
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            <button onClick={() => navigate('campaigns')} className="text-sm text-blue-600 hover:underline">View all</button>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
              {campaigns.map((c) => {
                ...
                return (
                  <div key={c.id} className="flex items-center gap-4 p-3 rounded-lg hover:bg-muted/50 cursor-pointer transition-colors" onClick={() => navigate(`campaign-detail/${c.id}`)}>
                    <div className="flex-1 min-w-0">
